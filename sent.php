...
$sql="SELECT messages.m_from,messages.m_to,messages.m_content,messages.m_sender,messages.m_date,users.userID FROM messages
         LEFT JOIN users ON messages.m_from=users.userID WHERE users.userID='".$_SESSION['user_id']."' ORDER BY m_id DESC";
		 $result=mysql_query($sql);

while($row=mysql_fetch_array($result)){
	$s="SELECT * FROM users WHERE userID='".$row['m_to']."'";
				$r=mysql_fetch_array(mysql_query($s));
                              echo  ' <div class="tr  ">
                                
                                <div class="td td-from">
                                    <a href="user.php?user='.$row['m_to'].'">'.$r['firstName'].' '.$r['lastName'].'</a>
                                </div>
